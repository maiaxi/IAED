...
int getday(char *command, int index){
    int i;
...
int yearequalyear(int tempmonth, int tempday){
    if (tempmonth < today.month){
        return 0;
    }
    else if (tempmonth == today.month){
        if (tempday < today.day){
            return 0;
        }
        else if(tempday == today.day){
            return 1;
        }
        else if (tempday > today.day){
            return 1;
...
int validate(Date temp){
    ...
    if (temp.year == today.year){
        return yearequalyear(temp.month, temp.day);
    }
...
void sort(char list[AIRPORTS_MAX][ID_SIZE], int size){
    ...
    for (i = 0; i < size; i++){                         /*if >0 , list[i]>list[i+1] */
        if (minors == size - 1){
            break;
        }
        else{
...
void emptyl(void){
    int i, empty; 
    char ids[AIRPORTS_MAX][ID_SIZE];
    for (i = 0; i < AIRPORTS_MAX; i++){
        empty = strcmp(list_airports[i].id, "\0\0");
        if (empty == 0){
            break;
        }
        strcpy(ids[i], list_airports[i].id);
        printf("%.*s", ID_SIZE, ids[i]);
    }
}      
...
void newApt(char * command){
    Airport apt;
    char id[ID_SIZE]; char country[COUNTRY_SIZE]; char city[CITY_SIZE];
    int i = 1 , e = 0, empty, commandlen, index;
    commandlen = strlen(command);
    index = getnext(command, i);
    for (i = index; e < ID_SIZE; i++){
        id[e] = command[i];
        e++;
    }
    id[e] = '\0';
...
int getminutes(char*command, int index){
    int i;
    for (i = index; command[i] != ':'; i++){
    }
    return i + 1;
...
void clearids(void){
    int i;
    for (i = 0; i < FLIGHTS_MAX; i++){
        strcpy(list_flights[i].id, "\0\0\0\0\0");
    }
    for (i = 0; i < AIRPORTS_MAX; i++){
        strcpy(list_airports[i].id, "\0\0");
    }
}
